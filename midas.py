...
class MidasApp:
    ...
    def handle_task(self, token, task):
        if 'completed' in task:
            if task['completed']:
                self.log(f"Task '{task['name']}' is already completed.")
        ...
        if 'canBeClaimedAt' in task and task['canBeClaimedAt'] is not None:
            ...
            if claim_result['success']:
                self.log(f"Claimed reward for task '{task['name']}'")
            else:
                self.log(f"Cannot claim reward for task '{task['name']}': {claim_result['error']}")
            return claim_result
        ...
        if 'waitTime' in task and task['waitTime']:
            self.log(f"Waiting {task['waitTime']} seconds to complete the task...")
            time.sleep(task['waitTime'])
        
        claim_result = self.claim_task(token, task['id'])
        if claim_result['success']:
            self.log(f"Completed and claimed reward for task '{task['name']}'")
        else:
            self.log(f"Cannot claim reward for task '{task['name']}' after waiting: {claim_result['error']}")
        return claim_result
    ...
    def update_visited(self, token):
        ...
        if response:
            self.log("Access updated successfully")
            return {'success': True}
        else:
            self.log(f"Cannot update access status. Error code: {response.status_code}")
            return {'success': False, 'error': f"HTTP Error: {response.status_code}"}
    ...
    def process_accounts(self):
        ...
        for index, init_data in enumerate(accounts, start=1):
            ...
            self.log(f"Account: {index}/{total_accounts}", Fore.GREEN)
            ...
            try:
                ...
                try:
                    ...
                    if streak_response.status_code == 200:
                        ...
                        self.log(f"Successfully checked in for day {streak_days}: reward {next_points} Points - {next_tickets} Tickets", Fore.GREEN)
                    elif streak_response.status_code == 400 and streak_info.get('message') == "Can't claim streak now":
                        self.log("You have already checked in today", Fore.YELLOW)
                    else:
                ...
                if not visited_result['success']:
                    self.log(f"Cannot update access status: {visited_result.get('error', 'Unknown error')}", Fore.RED)
                    ...
                    for i in range(tickets):
                        ...
                        try:
                            ...
                            if game_response:
                                ...
                                self.log(f"Tap successful: earned {points_earned} Points", Fore.GREEN)
                            else:
                ...
                if tasks_result['success']:
                    for task in tasks_result['tasks']:
                        self.log(f"Processing task: {task.get('name', 'Unknown task')}", Fore.YELLOW)
                        try:
                            ...
                            if task_result['success']:
                                if 'message' in task_result:
                                    ...
                                    self.log(f"Completed task: {task.get('name', 'Unknown task')}", Fore.GREEN)
                            else:
                                self.log(f"Cannot complete task: {task.get('name', 'Unknown task')} (not eligible yet)", Fore.YELLOW)
                        except Exception as e:
                            self.log(f"Error handling task '{task.get('name', 'Unknown task')}': {str(e)}", Fore.RED)
                            self.log(f"Task details: {json.dumps(task, indent=2)}", Fore.RED)
                else:
                    self.log(f"Cannot retrieve task list: {tasks_result['error']}", Fore.RED)
                
            except Exception as e:
                self.log(f"Error occurred for account {index}: {str(e)}", Fore.RED)
